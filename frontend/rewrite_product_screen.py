with open('/Users/shivavarma/Fresh_prown/Fresh_prowns/frontend/src/screens/ProductScreen.jsx', 'r') as f:
    text = f.read()

# We need to replace the entire return statement.
# Find where return ( starts

start_index = text.find("    return (\n        <div className=\"max-w-6xl mx-auto\">")
if start_index == -1:
    print("Could not find start index")
    exit(1)

imports = text[:start_index]
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                                {product.images.map((img, idx) => (
                                    <button
                                        ...
                                        className={`relative flex-shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-[8px] overflow-hidden transition-all duration-300 ${mainImageIndex === idx ? 'border-2 border-white opacity-100' : 'border border-transparent opacity-50 hover:opacity-100'}`}
                                    >
                                ...
                                <div className="pt-6 flex flex-col gap-4">
                                    {isAvailable && (
                                        <div className="flex justify-between items-center border border-[#333] rounded-[8px] p-2 bg-black/50">
                                            <span className="text-[#777] font-[600] text-[11px] uppercase tracking-widest pl-3 font-mono">Qty (kg)</span>
                                            <select
                                                value={qty}
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-[16px] md:py-[20px] rounded-[8px] font-[800] text-[13px] md:text-[14px] uppercase tracking-[0.1em] transition-all duration-300 ${isAvailable ? 'bg-[#dcdcdc] text-[#111] hover:bg-white hover:shadow-[0_0_20px_rgba(255,255,255,0.1)]' : 'bg-[#1a1a1a] text-[#555] cursor-not-allowed border border-[#333]'} mt-2`}
                                    >
                                        {isAvailable ? `Add ${selectedSize} to Cart` : 'Sold Out'}
                                    </button>
                                </div>
                            </div>
                        </div>
...
with open('/Users/shivavarma/Fresh_prown/Fresh_prowns/frontend/src/screens/ProductScreen.jsx', 'w') as f:
    f.write(imports + new_return)
print("Updated ProductScreen.jsx")
